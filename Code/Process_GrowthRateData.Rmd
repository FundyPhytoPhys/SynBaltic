...
MultiCultiGrowth900<-MultiCultiGrowth %>% 
  filter(Par_ue == 900) %>% 
...
MultiCultiGrowth600<-MultiCultiGrowth %>% 
  filter(Run != 77) %>% 
  filter(Par_ue == 600) %>% 
...
MultiCultiGrowthColour<-MultiCultiGrowth %>% 
  filter(Run == 34) %>% 
  mutate(Strain=case_when(Strain=="BA60G"~"PC-rich_060"))

MultiCultiGrowthTrans<-MultiCultiGrowth %>% 
...
MultiCultiGrowthPro<-MultiCultiGrowth %>% 
...
# Create preliminary plot - 900

```{r preliminary plot, warning = FALSE}
MultiCultiGrowth900 %>%
  ggplot() +
  geom_point(aes(x = PARPhotonDose_day, y = deltaOD_Lmu_corr,colour=as.factor(Photoperiod)), size = 3.5, alpha = 0.9, show.legend = T) +
  # geom_errorbar(aes(x = PARPhotonDose_day, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), alpha = 0.5) +
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("GrowthRate900_Plot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```

# Create preliminary plot - 600
...
MultiCultiGrowth600 %>%
  ...
  ggplot() +
  geom_point(aes(x = PARPhotonDose_day, y = deltaOD_Lmu_corr,colour=as.factor(Photoperiod)), size = 3.5, alpha = 0.9, show.legend = T) +
  geom_errorbar(aes(x = PARPhotonDose_day, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), alpha = 0.5, width = 0) +
  # scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  ggh4x::facet_nested(rows = vars(Strain)) +
  theme_bw()
```
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("GrowthRate600_Plot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```



# Create preliminary plot - Trans

```{r preliminary plot, warning = FALSE}
MultiCultiGrowthTrans %>%
  ggplot() +
  geom_point(aes(x = PARPhotonDose_day, y = deltaOD_Lmu_corr,colour=as.factor(Strain)), size = 3.5, alpha = 0.9, show.legend = T) +
  # geom_errorbar(aes(x = PARPhotonDose_day, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), alpha = 0.5) +
  scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(Run, Par_ue)) +
  theme_bw()
```
# Save plot 

```{r save plot}
ggsave(file = file.path(PlotsPath, paste("GrowthRateTrans_Plot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```



# Create preliminary plot - colour

```{r preliminary plot, warning = FALSE}
MultiCultiGrowthColour %>%
  ggplot() +
  geom_point(aes(x = PARPhotonDose_day, y = deltaOD_Lmu_corr,colour=as.factor(Strain)), size = 3.5, alpha = 0.9, show.legend = F) +
  geom_errorbar(aes(x = PARPhotonDose_day, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), alpha = 0.5, width = 0) +
  # scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  ggh4x::facet_nested(rows = vars(Par_ue), cols = vars(WL)) +
  theme_bw()
```
# Save plot 

```{r save plot}
ggsave(file = file.path(PlotsPath, paste("GrowthRateColour_Plot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
...
MultiCultiGrowthPro %>%
  ...
  # geom_errorbar(aes(x = PARPhotonDose_day, ymin = deltaOD_Lmu_corr - deltaOD_Lmu_se, ymax = deltaOD_Lmu_corr + deltaOD_Lmu_se), alpha = 0.5, width = 0) +
  # scale_colour_discrete(type=c("seagreen4", "palegreen3", "brown1", "brown4")) +
  ggh4x::facet_nested(rows = vars(Strain), cols = vars(WL, Par_ue)) +
  theme_bw()
```
...
```{r save plot}
ggsave(file = file.path(PlotsPath, paste("GrowthRatePro_Plot",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
