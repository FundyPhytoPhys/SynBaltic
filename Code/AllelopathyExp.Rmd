...
# colnames(GrowthData)
# Show in New Window
# [1] "6well_Plate"   "6well_PlateNo" "6well_WellNo"  "Sample_ID"     "ObsDate"       "Blank_13_A2"   "OD680"        
# [8] "OD750" 
# 
# colnames(CultureCatalog)
# [1] "6well_Plate"       "PlateNo"           "WellNo"            "96well_Plate"      "Sample_ID"         "Donor_GrowthPhase"
#  [7] "Content"           "Treatment"         "Donor_Strain"      "Target_Strain"     "Donor_Species"     "Targer_Species"   
# [13] "Start_Date"        "End_Date"          "PAR"               "Photoperiod"       "Salinity"          "Target_Medium"    
# [19] "Donor_Medium"      "Temperature"       "Notes"


GrowthMeta <- GrowthData %>%
  left_join(., CultureCatalog, by = c("Sample_ID"="Sample_ID", "6well_PlateNo"="PlateNo", "6well_WellNo"="WellNo"))
...
GrowthMeta %>%
  drop_na(Treatment) %>% 
  ggplot() +
  geom_point(aes(x = E_days, y =OD680, colour=as.factor(Donor_Strain)), show.legend = T) +
  geom_smooth(aes(x = E_days, y =OD680, colour=as.factor(Donor_Strain)), method = lm, show.legend = F) +
  ...
  ggh4x::facet_nested(cols = vars(Targer_Species), rows = vars(Content, Treatment), labeller = labeller(Ex_WL = label_both, strain = label_value, WL = label_both, Photoperiod = label_both, Par_ue = label_both)) +
  theme_bw()
```
