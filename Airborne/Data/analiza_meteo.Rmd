...
setwd("C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data")

#dane z meteo
plik_excel <- "C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data/dane_meteo20152020.xlsx"
...
plik_excel <- "C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data/dane_jakosciowe_zebrane.xlsx"
arkusze <- excel_sheets(plik_excel)
print(arkusze)
...
# Wybieranie danych
setwd("C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data")
plik_excel <- "C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data/meteo_2020_full.xlsx"
arkusze <- excel_sheets(plik_excel)
print(arkusze)
...
# Wybieranie danych
setwd("C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data")
plik_excel <- "C:/Users/sliwi/Documents/GitHub/SynBaltic/Airborne/Data/monthly_weather_average2020.xlsx"
arkusze <- excel_sheets(plik_excel)
print(arkusze)
...
merged_data<-merged_data %>% 
  rename(Month_1=Month) %>% 
    rename(Total_Count_1=Total_Count) %>% 
    rename(Mean_Count_1=Mean_Count) 

data <- merged_sea %>%
  select(-NNO3, -NPO4)

DataAll<-cbind(data, merged_data) %>% 
  rename(T_air = `Temp [°C]`) %>% 
  select(-c(Month_1, Total_Count_1, Mean_Count_1))
```
...
# Zakładam, że dane są już połączone:
# DataAll <- monthly_count_summary %>%
#   inner_join(average_meteo2020, by = c("Month" = "Months")) %>%
#   mutate(Month = factor(Month, levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", 
#                                           "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")))

# 🔹 Obliczamy współczynnik skalowania temperatury względem Mean_Count:
scale_factor <- max(DataAll$Mean_Count, na.rm = TRUE) / max(DataAll$T_air, na.rm = TRUE)

# Wykres
ggplot(DataAll, aes(x = Month)) +
  ...
  # Temperatura (prawa oś, przeskalowana)
  geom_line(aes(y = T_air * scale_factor, group = 1, color = "Temperature [°C]"), 
            size = 1.2, linetype = "dashed") +
  geom_point(aes(y = T_air * scale_factor, color = "Temperature [°C]"), size = 3) +
...
```
```{r Create plot, fig.height = 10, fig.width = 12, warning = FALSE}
...
plot_param <- function(data, param_name, y_label, x_label, color, legend_labels) {
  ...
  ggplot(data, aes(x = Month)) +
    geom_line(aes(y = Mean_Count, group = 1, color = legend_labels[1]), size = 0.6) +
    geom_point(aes(y = Mean_Count, color = legend_labels[1]), size = 5) +
    ...
    scale_y_continuous(breaks=seq(0,800 ,200),
      name = expression("Airborne microorganisms [cells m"^{-3}*"]"),
      sec.axis = sec_axis(~ (. - min1) / scale_factor + min2, name = y_label)
    ) +
    ...
    scale_color_manual(
      name = "Legend",
      values = setNames(c("indianred4", color), legend_labels)
    ) +
...
# Tworzymy wykresy z poprawnymi nazwami osi i kolorami
p1 <- plot_param(data, "T_sea", expression("T sea [°C]"), "Month", "steelblue", c("Airborne microorganisms", "T sea"))
p2 <- plot_param(data, "PP", expression("PP [mg m"^{-2}~d^{-1}*"]"), "Month", "lightgoldenrod3", c("Airborne microorganisms", "PP"))
p3 <- plot_param(data, "Sinice", expression("B-G Algae biomass [mg m"^{-3}*"]"), "Month", "lightseagreen", c("Airborne microorganisms", "B-G Algae"))
p4 <- plot_param(data, "Fitoplankton", expression("Phytoplankton biomass [mg m"^{-3}*"]"), "Month", "darkseagreen4", c("Airborne microorganisms", "Phytoplankton"))
...
plot_param <- function(DataAll, param_name, y_label, x_label, color, legend_labels) {
  min1 <- min(DataAll$Mean_Count, na.rm = TRUE)
  max1 <- max(DataAll$Mean_Count, na.rm = TRUE)
  min2 <- min(DataAll[[param_name]], na.rm = TRUE)
  max2 <- max(DataAll[[param_name]], na.rm = TRUE)
  scale_factor <- (max1 - min1) / (max2 - min2)
...
# Tworzymy wykresy z poprawnymi nazwami osi i kolorami
p1 <- plot_param(DataAll, "T_air", expression("T air [°C]"), "Month", "steelblue", c("Airborne microorganisms", "T air"))
p2 <- plot_param(DataAll, "PP", expression("PP [mg m"^{-2}~d^{-1}*"]"), "Month", "lightgoldenrod3", c("Airborne microorganisms", "PP"))
p3 <- plot_param(DataAll, "Sinice", expression("B-G Algae biomass [mg m"^{-3}*"]"), "Month", "lightseagreen", c("Airborne microorganisms", "B-G Algae"))
p4 <- plot_param(DataAll, "Fitoplankton", expression("Phytoplankton biomass [mg m"^{-3}*"]"), "Month", "darkseagreen4", c("Airborne microorganisms", "Phytoplankton"))
...
```{r Create plot, fig.height = 6, fig.width = 12, warning = FALSE}
#LOlLIPOP plot
...
ggplot(plot_frame, aes(x = reorder(parameter, coefficient), y = coefficient, color = color)) +
  geom_segment(aes(xend = parameter, y = 0, yend = coefficient), size = 1) +
  geom_point(size = 5) +  # Główki lolipopów
  scale_color_manual(values = c("p < 0.001" = "indianred4", "p < 0.01" = "orange2", "p < 0.05" = "gold", "p > 0.05" = "lightslategrey")) +
  scale_y_continuous(breaks = seq(-1, 1, by = 0.25), limits = c(-1, 1)) +  # Wymuszenie zakresu osi Y od -1 do 1
  coord_flip() +  # Obracamy wykres dla lepszej czytelności
  theme_bw() +
  theme(
