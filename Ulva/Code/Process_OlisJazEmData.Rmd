...
OLISSpectraFile <- "../Data/ImportedData/ImportedOlisData/Ulva_Imported_OlisData.Rds"
OLISSpectraFileName <- str_split(string = OLISSpectraFile, "/")[[1]][3] %>%
  str_remove(pattern = ".Rds")
OLISSpectra <- readRDS(OLISSpectraFile)  %>%
  ungroup()

JazFile <- "../Data/ImportedData/ImportedJazEmData/Ulva_Imported_JazEmData.Rds"
...
```{r}
JazDataRoundMeta<-JazDataRoundMetaWW %>% 
...
OLISSpectraMeta <- OLISSpectra %>%
  full_join(., JazDataRoundMeta, by = c("nm" = "MCEm_nm")) %>%
  unique()
```
...
OLISSpectraMetaPURWW <- OLISSpectraMeta %>%
  ...
  mutate(SumEmNormJaz = sum(EmNormJaz439)) %>% 
  mutate(PUR = 100*(PURNormSum/SumEmNormJaz)) %>% 
  ungroup() 

OLISSpectraMetaPUR <- OLISSpectraMetaPURWW 
```
...
# OLISSpectraMetaPUR %>%
#   filter(Strain == "Ulva1" | Strain == "Ulva2" | Strain == "Ulva3") %>% 
#   ggplot() +
#   geom_line(aes(x = nm, y = Absorbance, colour = as.factor(Strain)), show.legend = T, size = 0.4) +
#   geom_area(aes(x = nm, y = EmNormJaz439, fill = "gray60"), alpha = 0.6, show.legend = F) +
...
#   theme_bw() 
 ...
 OLISSpectraMetaPUR %>%
  filter(Strain == "Ulva1" | Strain == "Ulva2" | Strain == "Ulva3") %>% 
  ggplot() +
  geom_area(aes(x = nm, y = EmNormJaz439, fill = "lemonchiffon3"), alpha = 0.3, show.legend = F) +
    geom_area(aes(x = nm, y = PURNorm, fill = "darkseagreen"), alpha = 0.6, show.legend = F) +
  
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Strain)), show.legend = F, size = 0.8) +

  scale_color_manual(values = c("Ulva1" ="palegreen3", "Ulva2" ="darkslategray", "Ulva3" ="green4")) +
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -0.025, alpha = 0.5) +
  scale_fill_identity() +
...
```{r save plot}
ggsave(file = file.path(FigPath, paste("Fig_PURPARRatio2",".png",sep = "")), height=6, width= 8,  dpi = 300, limitsize = TRUE)
```


```{r Create plot, fig.height = 6, fig.width = 4}
 ...
 OLISSpectraMetaPUR %>%
  ...
  coord_cartesian(ylim = c (0, 1.2)) +
  #ggh4x::facet_nested(cols = vars(Strain), labeller = labeller(Ex_WL = label_value, WL = label_value)) +
  #ggh4x::facet_nested(cols = vars(factor(Strain, levels=c("Ulva1"))), labeller = label_parsed) +
  theme_bw() +
     theme(panel.grid.minor = element_blank(),
        panel.grid.major = element_blank(),
        axis.text = element_text(size=12),
...
```{r Create plot, fig.height = 8, fig.width = 6}
 ...
 OLISSpectraMetaPUR %>%
  ...
  geom_line(aes(x = nm, y = AbsNorm440, colour = as.factor(Strain), linetype = as.factor(Strain)), show.legend = T, size = 0.8) +
  ...
  scale_linetype_manual(values = c("Ulva1" ="solid", "Ulva2" ="dashed", "Ulva3" ="twodash")) +
  stat_wl_strip(aes(x = nm), ymin = -Inf, ymax = -0.025, alpha = 0.5) +
  scale_fill_identity() +
  labs(y = "Normalized absorbance and emission spectra", x = "Wavelength (nm)") +
  scale_y_continuous(breaks=seq(0, 1.2, by = 0.4)) +
...
```{r save plot}
#ggsave(file = file.path(FigPath, paste("Fig_PURPARRatio4",".png",sep = "")), height=10, width= 8,  dpi = 300, limitsize = TRUE)
```
