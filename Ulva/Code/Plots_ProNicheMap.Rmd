...
mapCircle<-ggplot() + 
  geom_rect(data=d3, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "dodgerblue4", color="midnightblue", alpha=0.5) +
  # geom_rect(data=d3, mapping=aes(xmin=x1, xmax=x2, ymin=y1, ymax=y2), fill = "lightskyblue1", color="lightskyblue3", alpha=0.5) +
  geom_map(data = world_coordinates2, map = world_coordinates2, aes(long, lat, map_id = region), color = "gray20", fill = "white", size = 0.2) +
   # geom_text(data=data_textA, aes(x=180, y=120, label=label), size=7, parse=TRUE) +
  scale_y_continuous(breaks=seq(-90, 90, by = 45)) +
